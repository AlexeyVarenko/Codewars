...
function convertHashToArray(hash){
  let massiv = Object.entries(hash).sort();
  return massiv
}
...
function covfefe(str){
 ...
 let newstr = str.replace(/coverage/gi, 'covfefe')
...
function stray(numbers) {
  ...
  return numbersSorted[0] == numbersSorted[1] ? numbersSorted[numbers.length -1] : numbersSorted[0]
}
...
function flattenAndSort(array) {
  let ABC=[].concat(...array)
  let result=ABC.sort((a, b) => a - b);
  return(result);
}